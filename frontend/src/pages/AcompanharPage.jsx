...
function urlDoProtocolo(protocolo) {
  return `/acompanhar/${encodeURIComponent(protocolo.trim())}`
}

function ConteudoAcompanhar({ protocolo, setProtocolo, detalhe, erro, carregando, onBuscar }) {
...
export default function AcompanharPage() {
  ...
  const consultar = useCallback(async (termo) => {
    ...
    try {
      const dados = await api.buscarPorProtocolo(valor)
      setDetalhe(dados)
    } catch (e) {
      setErro(e.message)
    } finally {
  ...
  useEffect(() => {
    const legado = searchParams.get('protocolo')
    if (legado && !protocoloUrl) {
      navigate(urlDoProtocolo(legado), { replace: true })
    }
  }, [searchParams, protocoloUrl, navigate])
  ...
  useEffect(() => {
    ...
    const valor = decodeURIComponent(protocoloUrl)
    setProtocolo(valor)
    consultar(valor)
  }, [protocoloUrl, consultar])

  const handleBuscar = () => {
    const termo = protocolo.trim()
    if (!termo) {
      return
    }

    const protocoloNaUrl = protocoloUrl ? decodeURIComponent(protocoloUrl) : ''
    if (termo !== protocoloNaUrl) {
      navigate(urlDoProtocolo(termo))
      return
    }
